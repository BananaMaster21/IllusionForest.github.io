...
function moving () {
   if (side === "ArrowLeft") {
    ...
    //falling
    if (caracterY < ground && onGround === 0 && upTimer === 0 || fallStop === 0 && jump === 0) {
...
function obstacleFloor (image, x, y, width, height) {
   ...
   if (y <= caracterY && caracterX + caracterW /2 > x && caracterX + caracterW /2 < x + width) {fallStop = 1;}else {if (onGround === 1) {fallStop = 1;}else {fallStop = 0;}}
}
